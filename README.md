...
| [`.github/workflows/docs-site-soft-destroy.yml`](.github/workflows/docs-site-soft-destroy.yml) | Static export, Terraform static + S3 sync + CloudFront invalidation, **Route53 cutover to CloudFront**, **`terraform destroy`** Fargate/ECS. CloudFront cert: **`TF_ACM_CERTIFICATE_ARN`** when **`AWS_REGION`** is **`us-east-1`**, else optional **`TF_CLOUDFRONT_ACM_CERTIFICATE_ARN`** (us-east-1 ACM). |
